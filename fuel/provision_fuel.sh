...
if [ -f /home/jenkins/.ssh/known_hosts ] ; then
  ssh-keygen -f /home/jenkins/.ssh/known_hosts  -R 10.20.0.2
fi

if [ -f /root/.ssh/known_hosts ] ; then
  ssh-keygen -f /root/.ssh/known_hosts  -R 10.20.0.2
fi
...
offending_key=`sshpass -pr00tme ssh -o StrictHostKeyChecking=no root@${fuel_master} ${ssh_cmd} 2>&1 | awk -F ':' '/Offending ECDSA key/ {print($2)}'`
if [[ $? != 0 ]] ; then
  ...
  # retry
  if ! sshpass -pr00tme ssh -o StrictHostKeyChecking=no root@${fuel_master} ${ssh_cmd} ; then
    echo ERROR: failed to add ssh key to fuel known hosts
    exit -1
  fi
fi
