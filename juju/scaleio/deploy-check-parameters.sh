...
m1=$(juju add-machine --constraints "instance-type=t2.medium" 2>&1 | awk '{print $3}')
echo "Machine created: $m1"
m2=$(juju add-machine --constraints "instance-type=t2.medium" 2>&1 | awk '{print $3}')
echo "Machine created: $m2"
m3=$(juju add-machine --constraints "instance-type=t2.medium" 2>&1 | awk '{print $3}')
echo "Machine created: $m3"
#m4=$(juju add-machine --constraints "instance-type=t2.medium" 2>&1 | awk '{print $3}')
#echo "Machine created: $m4"
#m5=$(juju add-machine --constraints "instance-type=t2.medium" 2>&1 | awk '{print $3}')
...
for instance in $instances ; do
  if [[ $i != 0 ]] ; then
    ...
    create_attach_volume $instance 100 "/dev/xvdf" $zone
    create_attach_volume $instance 100 "/dev/xvdg" $zone
  fi
  ((++i))
done
