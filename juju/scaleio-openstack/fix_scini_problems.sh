...
created=0
if [[ -z "$m1" && -z "$m2" ]] ; then
  m1=$(juju add-machine --constraints "instance-type=m4.large root-disk=40G" 2>&1 | awk '{print $3}')
  echo "Machine created: $m1"
  m2=$(juju add-machine --constraints "instance-type=m4.large root-disk=40G" 2>&1 | awk '{print $3}')
