...
if [[ "$deploy_from" == github ]] ; then
  if [[ "$jver" == 1 ]] ; then
    ...
    # version 2
    params="$WORKSPACE/juju-scaleio/$SERIES/"
  fi
else
...
juju-add-unit nova-compute --to $m2
...
juju-add-unit scaleio-sdc --to $m2

echo "INFO: Deploy subordinate to OpenStack"
juju-deploy ${params}scaleio-openstack
juju-set scaleio-openstack "provisioning-type=thick"
...
juju-add-unit scaleio-mdm --to $m4
juju-add-unit scaleio-mdm --to $m5

echo "INFO: Deploy SDS"
juju-deploy ${params}scaleio-sds --to $m3
juju-add-unit scaleio-sds --to $m4
juju-add-unit scaleio-sds --to $m5
